...
export class AtlasIndex extends BaseAtlasClass {
    ...
    async atomInformation(ids) {
        ...
        const responseJson = await this.apiCall(`/v1/project/atoms/get`, "POST", body).then((d) => d.json());
    ...
    async projections() {
        if (this._projections) {
            ...
            const projections = project_info.atlas_indices?.find((d) => d.id === this.id)
                ?.projections || [];
            this._projections = projections.map((d) => new AtlasProjection(d.id, {
                index: this,
