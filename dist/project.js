...
export class AtlasProject extends BaseAtlasClass {
    ...
    */
    ...
    async update_indices(rebuild_topic_models = false) {
        await this.apiCall(`/v1/project/update_indices`, "POST", { 'project_id': this.id,
    ...
    async createIndex(options) {
        let defaults = {
            ...
            'projection_hyperparameters': JSON.stringify({ 'n_neighbors': 64, 'n_epochs': 64, 'spread': 1 }),
    ...
    async delete_data(ids) {
        ...
        await this.user.apiCall("/v1/project/data/delete", "POST", { 'project_id': this.id, 'datum_ids': ids });
    }
    validate_metadata() {
        // validate metadata
    }
